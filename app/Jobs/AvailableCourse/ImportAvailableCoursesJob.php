...
use App\Services\AvailableCourse\Operations\ProcessImportService;
use Throwable;

class ImportAvailableCoursesJob implements ShouldQueue
{
    ...
    private function processAllChunks(float $progressPerChunk): void
    {
        ImportStaging::where('task_id', $this->task->id)
            ->where('import_type', 'available_course')
            ->orderBy('id')
            ->chunk(self::CHUNK_SIZE, function ($stagedRows) use ($progressPerChunk) {
                ...
                $this->setProgress(
                    ...
                    sprintf(
                        'Processing chunk %d of %d (%s rows processed, %s succeeded, %s failed)',
                        ...
                        number_format($this->task->result['summary']['succeeded'] ?? 0),
    ...
    private function mergeChunkResultsAndIncrement(array $chunkResults): void
    {
        DB::transaction(function () use ($chunkResults) {
            ...
            $result['summary'] = [
                ...
                'succeeded' => ($currentSummary['succeeded'] ?? 0) + ($chunkResults['summary']['succeeded'] ?? 0),
    ...
    private function finalizeImport(): void
    {
        ...
        $this->completeProgress(
            ...
            sprintf(
                'Import completed: %s processed - %s succeeded, %s failed',
                number_format($summary['total_processed'] ?? 0),
                number_format($summary['succeeded'] ?? 0),
