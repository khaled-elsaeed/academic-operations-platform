  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.success) {
        ...
        $('#users-value, #admin-value').text('N/A');
        $('#users-last-updated, #admin-last-updated').text('N/A');
      }
      // Hide loading state
...
function editUser(userId) {
    ...
    $.get(`{{ route('users.show', ':id') }}`.replace(':id', userId))
        .done(function(response) {
            if (response.success) {
                ...
                // Load roles dropdown with preselected values
                loadRoles();
                
                // Set selected role after a short delay to ensure Select2 is initialized
                setTimeout(function() {
                    // For single role selection, take the first role or empty if no roles
                    const selectedRole = user.roles.length > 0 ? user.roles[0].name : '';
                    $('#roles').val(selectedRole).trigger('change');
                }, 100);
...
$('#userForm').on('submit', function(e) {
    ...
    const url = isUpdate ? `{{ route('users.update', ':id') }}`.replace(':id', currentUserId) : '{{ route("users.store") }}';
